...
//        }
    ...
    private func setModeToFuntik() {
        // Switch to WebView mode and provide the Unity interface link so the UI can load it.
        UserDefaults.standard.set("WebView", forKey: "app_mode")
        let link = "https://play.unity.com/en/games/10e67e7c-11b1-4756-babf-3ff6c8fbad93/character-managment"
        UserDefaults.standard.set(link, forKey: "saved_url")
        // Also set temp_url and notify loaders that a URL is available to load immediately
        UserDefaults.standard.set(link, forKey: "temp_url")
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: NSNotification.Name("UpdateUI"), object: nil)
            NotificationCenter.default.post(name: NSNotification.Name("LoadTempURL"), object: nil, userInfo: ["tempUrl": link])
        }
    }
...
class SplashViewModel: ObservableObject {
    ...
    private func checkInternetAndProceed() {
        ...
        // If we already have a saved URL (from previous run or AppDelegate fallback),
        // open it immediately in the web view so the app doesn't show a black native host.
        if let saved = UserDefaults.standard.string(forKey: "saved_url"), !saved.isEmpty {
    ...
    private func processConversionData() {
        ...
        if UserDefaults.standard.string(forKey: "app_mode") == "Funtik" {
            // Treat legacy "Funtik" mode as the Unity web interface: switch to WebView.
    ...
    func sendConfigRequest() {
        guard let url = URL(string: "https://charactermanagment.com/config.php") else {
            handleConfigError()
            return
        }
    ...
    private func setModeToFuntik() {
        // Instead of launching the native Funtik UI, open the Unity web page in the web view.
        UserDefaults.standard.set("WebView", forKey: "app_mode")
        UserDefaults.standard.set(true, forKey: "hasLaunched")

        let link = "https://play.unity.com/api/v1/games/game/10e67e7c-11b1-4756-babf-3ff6c8fbad93/build/latest/frame"
        if let url = URL(string: link) {
            ...
            // Fallback: ensure saved_url/temp_url are set and instruct UI to load web view
            UserDefaults.standard.set(link, forKey: "saved_url")
            UserDefaults.standard.set(link, forKey: "temp_url")
            DispatchQueue.main.async {
                NotificationCenter.default.post(name: NSNotification.Name("LoadTempURL"), object: nil, userInfo: ["tempUrl": link])
            ...
            if viewModel.showNotificationScreen {
                ...
                case .webView:
                    if let url = viewModel.webViewURL {
                        CoreInterfaceView(destinationLink: url)
                    } else {
                        ContentView()
                            .environmentObject(appState)
                    }
...
struct ChickenCareApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            // Always start from the SplashView so startup logic decides which screen to show.
            SplashView()
                .environmentObject(appState)
        }
    }
...
struct ContentView: View {
    ...
    var body: some View {
        Group {
            if let webURL = resolvedWebViewURL() {
                CoreInterfaceView(destinationLink: webURL)
            } else {
...
//        .font(.system(.body, design: .rounded))
    ...
    private func resolvedWebViewURL() -> URL? {
        // Prefer the freshest temp_url (conversion/push) while we're online.
        if NetworkMonitor.shared.isConnected,
           let tempString = UserDefaults.standard.string(forKey: "temp_url"),
           let tempURL = URL(string: tempString) {
            return tempURL
        }
...
struct MainBrowserView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        context.coordinator.lastRequestedURL = destinationLink
    ...
    func updateUIView(_ browser: WKWebView, context: Context) {
        if context.coordinator.lastRequestedURL != destinationLink {
            context.coordinator.lastRequestedURL = destinationLink
            browser.load(URLRequest(url: destinationLink))
        }
    }
    
    func makeCoordinator() -> MainBrowserCoordinator {
        MainBrowserCoordinator(manager: manager)
    }
    
}

final class MainBrowserCoordinator: BrowserDelegateManager {
...
struct CoreInterfaceView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            MainBrowserView(destinationLink: activeURL)
        }
        .preferredColorScheme(.dark)
        .onAppear {
            synchronizeInterfaceURL()
        }
        .onReceive(NotificationCenter.default.publisher(for: Notification.Name("LoadTempURL"))) { notification in
            handleLoadTempURL(notification: notification)
        }
    }
    ...
    private func handleLoadTempURL(notification: Notification) {
        guard NetworkMonitor.shared.isConnected else { return }
        
        let userInfo = notification.userInfo as? [String: Any]
        let rawURL = userInfo?["tempUrl"] as? String ?? UserDefaults.standard.string(forKey: "temp_url")
        
        guard let urlString = rawURL, let freshURL = URL(string: urlString) else {
            return
        }
    ...
    private func consumeTempURL() -> URL? {
        guard let candidate = UserDefaults.standard.string(forKey: "temp_url"),
              let url = URL(string: candidate) else {
            return nil
        }
        
        UserDefaults.standard.removeObject(forKey: "temp_url")
        UserDefaults.standard.set(candidate, forKey: "saved_url")
        return url
    }
}
